...
# @app.route('/sd')
def sd(epo, device_id, scale_left, scale_right, prompt, negative_prompt):
    # args = request.args
    # set scale list
    w_list = []
    ...
    for i in range(int(epo)):
        ...
        generators = []
        seed_list = []
        for i in range(int(n_images_per_prompt)):
...
if __name__ == '__main__':
    epo = math.ceil(epo)
    keywords = ' elegant, highly detailed, 4 k'
    result = sd(epo, device_id, scale_left, scale_right, prompt + keywords, negative_prompt)
    MyFile = open('json_result.json', 'w')
    MyFile.write(result)
    MyFile.close()
