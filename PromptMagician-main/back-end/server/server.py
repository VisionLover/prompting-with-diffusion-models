...
dataset = datasets.load_dataset('poloclub/diffusiondb', '2m_first_10k')
dataset.save_to_disk('poloclub/diffusiondb_2m_first_10k')
...
db_image_features, db_text_features, db_combined_feature, db_filter_text_list, db_id_list = [i[:9000] for i in data]
...
def get_image_overview():
    ...
    json_string = open('/content/PromptMagician-main/back-end/server/json_result.json').readlines()[0]
    sd_data = json.loads(json_string)
    # prompt = ''
    ...
    for j in range(n_sd * n_images_per_prompt):
        ...
        sd_origin_image_list.append(base64_to_image(sd_data[j]['img']))
        sd_process_text_list.append(process_prompt)
        sd_seed_list.append(sd_data[j]['seed'])
        sd_cfg_list.append(sd_data[j]['guidanceScale'])
    ...
    print(all_process_img_list[0].shape)
...
def get_image_rating():
    ...
    #Encode an image:
    json_string = open('/content/PromptMagician-main/back-end/server/final_json_result.json').readlines()[0]
    json_array = json.loads(json_string)
    img_emb = model.encode([base64_to_image(json_array['image'][i]['img']) for i in range(len(all_process_img_list))])
...
if __name__ == '__main__':
    ...
    result = get_image_overview()
    MyFile = open('final_json_result.json', 'w')
    MyFile.write(result)
    MyFile.close()
    rate = get_image_rating()
    print(len(rate))
    print('average rate for generated images: ', torch.mean(rate[:10]))
    print('average rate for searched images: ', torch.mean(rate[10:]))
